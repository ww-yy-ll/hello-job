			...
			<view class="">
				<u--form labelPosition="top" :model="form" ref="form" :labelStyle="labelStyle">
					<u-form-item label="文件名" :required="true" prop="name" ref="name" borderBottom>
						<u--input :disabled="false" v-model="form.name" border="none" placeholder="请输入,如我的简历···"></u--input>
					</u-form-item>
					<u-form-item label="备注" :required="true" prop="content" :borderBottom="false" ref="content">
						<u--textarea placeholder="可填写该附件的说明,让招聘者更好地了解" v-model="form.content" count height="250rpx" maxlength="200"></u--textarea>
					</u-form-item>
				</u--form>
			</view>
		</view>
		<view class="b-r-12 bg-fff box-shadow p-40 m-b-30">
			<view class="p-b-30">
				<u--form labelPosition="top" :model="form" ref="form" :labelStyle="labelStyle3">
					<u-form-item label="附件上传(可上传jpg、png,小于5M)" :required="true" prop="zsImage" :borderBottom="false" ref="zsImage">
						<u-upload
...
<script>
	...
	export default {
		...
		watch:{
			'form.name'(){
				const { name, content, zsImage } = this.form
				if(name.length!==0 && content.length!==0 && zsImage.length!==0 ) {
					this.allCheck = true
				} else {
					this.allCheck = false
				}
